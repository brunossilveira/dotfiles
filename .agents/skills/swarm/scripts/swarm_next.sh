...
# Absolute, so every command and prompt path it prints works from any
# repository the run happens to be in.
...
for b in $(open_blockers "$dir"); do
    emit handle_blocker \
        ...
        "COMMAND: $S/swarm_blocker.sh resolve $b <detail>"
    exit 0
done
...
for g in $(open_gates "$dir"); do
    emit await_approval \
        ...
        "COMMAND_ON_APPROVAL: $S/swarm_gate.sh approve $g <detail>" \
        "COMMAND_ON_REJECTION: $S/swarm_gate.sh reject $g <reason>"
    exit 0
done
...
if [[ -z "$stories" ]]; then
    if [[ "$plan_state" == "rejected" ]] && (( plan_rejections >= REWORK_LIMIT )); then
        emit open_blocker \
            ...
            "COMMAND: $S/swarm_blocker.sh open plan-rejected plan run \"plan rejected $plan_rejections times\""
        exit 0
    fi
    emit run_analyst \
        "ROLE: analyst" \
        ...
        "PROMPT: $SKILL/role-templates/analyst.prompt" \
        "RECORD_WITH: $S/swarm_story.sh add <story-id> <title> [--depends-on <id>] [--risky]" \
        "THEN: $S/swarm_gate.sh request plan plan run \"approve the story split and dependency order\""
    exit 0
fi
...
if [[ "$plan_state" != "approved" ]]; then
    if [[ "$plan_state" == "rejected" ]]; then
        ...
        emit request_plan_approval \
            ...
            "COMMAND: $S/swarm_gate.sh request plan plan run \"approve the story split and dependency order\""
    fi
    exit 0
fi
...
story_action() {
    ...
    if [[ "$pr_state" == "merged" ]]; then
        echo "record_merge|$s||the pull request is merged|$S/swarm_story.sh record $s merged done"
        return 0
    fi
    ...
    if [[ -n "$parent" ]] && stage_done "$dir" "$parent" merged; then
        ...
        if [[ -n "$cur_base" && "$cur_base" != "$base" ]]; then
            echo "restack|$s|merger|parent $parent merged; this branch is on $cur_base|$S/swarm_worktree.sh restack $s"
            return 0
        fi
    fi
    ...
    stage_done "$dir" "$s" implementation || {
        echo "run_implementer|$s|implementer|the story is approved and unimplemented|$S/swarm_story.sh record $s implementation done <sha>"
        return 0
    }

    [[ -z "$pr" ]] && {
        echo "open_draft_pr|$s||implementation exists with no pull request|$S/swarm_pr.sh open $s \"$(story_field "$dir" "$s" title)\""
        return 0
    }
    ...
    [[ -z "$code_v" ]] && {
        echo "run_code_reviewer|$s|code-reviewer|implementation has no review|$S/swarm_story.sh record $s code-review <accepted|changes-requested> <sha>"
        return 0
    }
    [[ -z "$adv_v" ]] && {
        echo "run_adversarial_review|$s|codex|no independent pass on this branch yet|$S/swarm_review.sh run $s"
        return 0
    }
    ...
    if [[ "$code_v" == "changes-requested" || "$adv_v" == "changes-requested" ]]; then
        ...
        if (( cycles >= REWORK_LIMIT )); then
            echo "open_blocker|$s||review requested changes after $cycles rework cycles; stop instead of another attempt|$S/swarm_blocker.sh open rework-$s rework $s \"$cycles rework cycles\""
        else
            echo "rework_implementer|$s|implementer|review requested changes (cycle $((cycles + 1)) of $REWORK_LIMIT)|$S/swarm_story.sh rework $s \"review requested changes\" then re-run the implementer"
        fi
        return 0
    fi

    stage_done "$dir" "$s" cleanup || {
        echo "run_cleaner|$s|cleaner|review accepted; cleanup has not run|$S/swarm_story.sh record $s cleanup done <sha>"
        return 0
    }

    if [[ "$risky" == "true" ]] && ! stage_done "$dir" "$s" hardening; then
        echo "run_hardener|$s|hardener|story marked risky and not hardened|$S/swarm_story.sh record $s hardening done <sha>"
        return 0
    fi

    local arch_v; arch_v="$(stage_verdict "$dir" "$s" architecture)"
    [[ -z "$arch_v" ]] && {
        echo "run_architect|$s|architect|no architecture review yet|$S/swarm_story.sh record $s architecture <accepted|changes-requested> <sha>"
        return 0
    }

    if [[ "$arch_v" == "changes-requested" ]] && ! stage_done "$dir" "$s" senior-implementation; then
        echo "run_senior_implementer|$s|senior-implementer|architecture review requested changes|$S/swarm_story.sh record $s senior-implementation done <sha>"
        return 0
    fi

    [[ "$pr_state" == "draft" ]] && {
        echo "mark_pr_ready|$s||every stage cleared; the PR is still a draft|$S/swarm_pr.sh ready $s"
        return 0
    }

    case "$ci" in
        failing)
            echo "run_implementer|$s|implementer|CI is failing on the pull request|$S/swarm_story.sh record $s implementation done <sha>"
            return 0 ;;
        pending|"")
            echo "await_ci|$s||waiting on CI for the pull request|$S/swarm_pr.sh sync $s"
            return 0 ;;
    esac
...
if [[ -z "$primary" ]]; then
    if [[ -n "$stories" ]] && ! echo "$stories" | while read -r s; do stage_done "$dir" "$s" merged || echo no; done | grep -q no; then
        emit complete_run \
            ...
            "COMMAND: $S/swarm_run.sh complete"
    else
...
emit "$action" \
    ...
    "REASON: $reason" \
    "COMMAND: $command" \
    ${role:+"PROMPT: $SKILL/role-templates/$role.prompt"} \
